<?php

namespace Tests\Feature\Livewire\Admin\Tagihan;

use Tests\TestCase;
use App\Models\Santri;
use Livewire\Livewire;
use App\Models\Tagihan;
use App\Models\Catering;
use App\Models\Diniyyah;
use App\Models\Syahriyyah;
use App\Livewire\Santri\Tagihan\TagihanTable;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Support\Facades\Auth;

class TagihanTableSantriTest extends TestCase
{
  use RefreshDatabase;

  protected Santri $santri;
  protected $tagihan;

  public function setUp(): void
  {
    parent::setUp();

    $this->santri = $this->createSantri([
      'nama_santri' => 'Budi'
    ]);

    $this->tagihan = $this->createTagihanForSantri($this->santri);

    $this->actingAs($this->santri);
  }

  public function test_santri_can_see_tagihan_table()
  {
    Livewire::test(TagihanTable::class)
      ->assertViewIs('livewire.santri.tagihan.tagihan-table')
      ->assertStatus(200);
  }

  public function test_unauthorized_user_cannot_see_tagihan_table()
  {
    Auth::logout($this->santri);

    $response = $this->get(route('tagihan-santri'));

    $response->assertRedirect(route('login'));
  }

  public function test_verify_field_tagihan_is_valid()
  {
    Livewire::test(TagihanTable::class)
      ->assertSee($this->tagihan->jenis_tagihan)
      ->assertSee($this->tagihan->semester ? $this->tagihan->semester : '-')
      ->assertSee($this->tagihan->bulan ? $this->tagihan->bulan : '-')
      ->assertSee($this->tagihan->tahun_ajaran)
      ->assertSee($this->tagihan->formatToRupiah('nominal'))
      ->assertSee(ucwords($this->tagihan->status));
  }

  public function test_failed_table_tagihan_is_empty()
  {
    $this->tagihan->delete();
    Livewire::test(TagihanTable::class)
      ->assertSee('Tidak Ada Tagihan Terima Kasih');
  }

  protected function createSantri(array $attributes = [])
  {
    return Santri::factory()->create($attributes);
  }

  protected function createTagihanForSantri($santri, array $attributes = [])
  {
    return Tagihan::create(array_merge([
      'id_santri' => $santri->id,
      'jenis_tagihan' => 'syahriyyah',
      'nominal' => 100000,
      'status' => 'belum lunas',
      'tahun_ajaran' => now()->year,
      'tgl_tagihan' => now()->subDays(1),
      'semester' => 1,
    ], $attributes));
  }
}
