<header class="sticky top-0 z-9 flex w-full bg-[#f4f4f5] drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none h-14">
                                ...
                                @foreach (auth('admin')->user()->notifications as $notification)
                                    <li>
                                        <a class="flex flex-col gap-2.5 border-t border-stroke px-4.5 py-3 hover:bg-gray-2 dark:border-strokedark dark:hover:bg-meta-4"
                                            ...
                                            <p class="text-sm">
                                                <span
                                                    class="text-black dark:text-white">{{ $notification->data['message'] }}</span>
                                            </p>
                ...
                <a class="flex items-center gap-4" href="#" @click.prevent="dropdownOpen = ! dropdownOpen">
                    <span class="hidden text-right lg:block">
                        <span class="block text-sm font-medium text-black dark:text-white">
                            {{ auth('admin')->check() ? auth('admin')->user()->nama_admin : auth('web')->user()->nama_santri }}
                        </span>
                        <span class="block text-xs font-medium">
                            {{ auth('admin')->check() ? 'Admin' : 'Santri' }}
                        </span>
                    </span>
