<div>
    <x-modals name="edit-rekening-modal" header="Edit Rekening Pondok">
        <form class="space-y-4" wire:submit='update'>
            <div class="w-full">
                ...
                <div wire:ignore>
                    <select class="bank-select-edit w-full" style="width: 100%">
                        <option value=""></option>
...
<script>
    ...
    $('.bank-select-edit').on('change', function (e) {
        const data = $('.bank-select-edit').select2("val");
        @this.id_bank = data;
        });
    
    window.addEventListener('update-select2', value => {
        const idBank = value.detail.idBank;
        $('.bank-select-edit').val(idBank).trigger('change');
    })
